...
<Card class="my-2" padding="xl" size="full">
  <div class="flex justify-between items-center mb-4">
    <h5 class="text-xl font-bold leading-none text-gray-800 dark:text-gray-100">
      Your linked banks
    </h5>
  </div>
  <Listgroup items={data} let:item class="border-0 dark:!bg-transparent">
    <div class="flex items-center space-x-4">
      <Avatar src={item.logo ? ('data:image/png;base64, ' + item.logo) : "./bank-icon.png"} alt={item.name} class="flex-shrink-0" />
      ...
      <div>
        <Icon name="dots-vertical-outline" id={item.institutionId} class="ddark:text-white" />
        <Dropdown triggeredBy={"#"+item.institutionId}>
          <DropdownItem on:click={()=> toast.promise(
            deleteAccount(item.institutionId),
            {
